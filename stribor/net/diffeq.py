...
class DiffeqMLP(DiffeqConcat):
    def __init__(self, *args, **kwargs):
        super().__init__()
        self.net = st.net.MLP(*args, **kwargs)

class DiffeqEquivariantNet(DiffeqConcat):
    def __init__(self, *args, **kwargs):
        super().__init__()
        self.net = st.net.EquivariantNet(*args, **kwargs)

class DiffeqSelfAttention(DiffeqConcat):
    def __init__(self, *args, **kwargs):
        super().__init__()
        self.net = st.net.SelfAttention(*args, **kwargs)

class DiffeqDeepSet(DiffeqConcat):
    def __init__(self, in_dim, hidden_dims, out_dim, **kwargs):
        super().__init__()
        self.net1 = st.net.MLP(in_dim + 1, hidden_dims[:-1], hidden_dims[-1])
        self.net2 = st.net.MLP(hidden_dims[-1] + in_dim + 1, [], out_dim)
...
class DiffeqZeroTraceDeepSet(nn.Module):
    def __init__(self, in_dim, hidden_dims, out_dim, pooling='max', return_log_det_jac=True, **kwargs):
        ...
        self.interaction = st.net.ExclusivePooling(in_dim, hidden_dims, out_dim // in_dim, pooling)
        self.return_log_det_jac = return_log_det_jac
