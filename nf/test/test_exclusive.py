...
def test_exclusive_set_net_zero_trace(input_dim, hidden_dims, out_dim, pooling):
    ...
    dim = input_dim[-1]
    model = nf.net.DiffeqZeroTraceDeepSet(dim, hidden_dims, dim * out_dim, pooling, return_log_det_jac=False)
...
def test_exclusive_set_net_masking(input_dim, pooling):
    ...
    model = nf.net.DiffeqZeroTraceDeepSet(input_dim[-1], [32], input_dim[-1] * 4, pooling, return_log_det_jac=False)
