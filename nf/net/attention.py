...
def attention(query, key, value, n_heads=1, mask_diagonal=False, mask=None):
    *rest, D = query.shape
    ...
    att = nf.util.safe_softmax(att, -1)
...
class Attention(nn.Module):
    def __init__(self, in_dim, hidden_dims, out_dim, n_heads=1, mask_diagonal=False, **kwargs):
        ...
        self.key = nf.net.MLP(in_dim, hidden_dims[:-1], hidden_dims[-1])
        self.query = nf.net.MLP(in_dim, hidden_dims[:-1], hidden_dims[-1])
        self.value = nf.net.MLP(in_dim, hidden_dims[:-1], hidden_dims[-1])
